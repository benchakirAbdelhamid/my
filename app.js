...
function overlay(count){
    ...
    link.setAttribute('onclick' , `limk( )` ) 
    ...
    // ----------------------------------------
        ...
        linkZoom.setAttribute('href' , img.getAttribute('data-urlFull') ) 
        linkZoom.setAttribute('id',`zoom${count}`)
        linkZoom.setAttribute('onclick' , `linkZoom( ${count})` ) 
        linkZoom.className = 'icon_linkZoom'
    
        const img_linkZoom = document.createElement('img')
        img_linkZoom.src = 'zoom.svg'
        linkZoom.appendChild(img_linkZoom)
        parent_popupBox.appendChild(linkZoom)
...
function limk(c){
    ...
    const IMG_URL =  img_download.src
...
function linkZoom(c){
    console.log(c)
    const img_download = document.querySelector(".parent_popupBox .popup-box1 .img_pupop")
    const id_zoom = document.querySelector(`#zoom${c}`)
    id_zoom.setAttribute('href' , img_download.src ) 
}
